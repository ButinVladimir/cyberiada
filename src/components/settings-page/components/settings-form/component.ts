...
import { LONG_NUMBER_FORMATS, THEMES, Language, LongNumberFormat, Theme, BaseComponent } from '@shared/index';
...
export class SettingsForm extends BaseComponent {
  ...
  private renderContent(desktop: boolean) {
    ...
    const settingsFormClasses = classMap({
      'settings-form': true,
      desktop: desktop,
      mobile: !desktop,
    });
    ...
    return cache(html`
      <form id="settings-form" @submit=${this.handleSubmit}>
        <div class=${settingsFormClasses}>
          <sl-select
