...
export class ShareServerDescriptionEffectRenderer implements IDescriptionEffectRenderer {
    ...
    return html`
      <p>
        ${COMMON_TEXTS.parameterValue(
          ...
          PROGRAM_DESCRIPTION_TEXTS.valuePerSec(html`<span data-value=${VALUES.money}></span>`),
        )}
      </p>
      <p>
        ${COMMON_TEXTS.parameterValue(
          REWARD_PARAMETER_NAMES[RewardParameter.developmentPoints](),
          PROGRAM_DESCRIPTION_TEXTS.valuePerSec(html`<span data-value=${VALUES.developmentPoints}></span>`),
        )}
      </p>
    `;
  };
  ...
  public recalculateValues() {
    ...
    const money = program.calculateMoneyDelta(usedCores, this._autoscalableProcessRam, MS_IN_SECOND);
    const developmentPoints = program.calculateDevelopmentPointsDelta(
      usedCores,
      this._autoscalableProcessRam,
      MS_IN_SECOND,
    );
